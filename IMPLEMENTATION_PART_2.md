...
The following static mock datasets are established for local auth scope mapping and catalog indexing. They must be loaded into memory or local storage fallbacks to prevent dependency on external Supabase modifications during local testing.
...
### 1.2 Internal Product Catalog (20 Items)
A collection of exactly 20 items, distributed across 4 distinct categories. Product image fields link to high-quality, un-cropped, design-aligned assets:
...
- **Task Checklist:**
  - [ ] Add the HSL/HEX root tokens to `src/app/globals.css` (or `index.css` equivalent context) matching the color schema.
  - [ ] Declare Canvas Mist (`#f2f4f5`), Pure White (`#ffffff`), Ink Black (`#000000`), Faint Border (`#ebebeb`), Muted Gray (`#787574`), and Shop Violet (`#5433eb`).
  - [ ] Map type scales for caption (11px), body-sm (12px), body (14px), body-lg (16px) with tight negative tracking configurations.
  - [ ] Configure Tailwind custom transitions and shadow keys (`--shadow-sm`, `--shadow-sm-2`, `--shadow-lg`, `--shadow-lg-2`).
- **Success Criteria:**
...
- **Task Checklist:**
  - [ ] Create a `SidebarRail` component utilizing a fixed vertical positioning layout (`fixed left-0 top-0 h-screen w-16`).
  - [ ] Configure icon zones: 24px outlined Lucide icons centered within 48px square tap spots.
  - [ ] Apply state changes: active states fill the background container with Canvas Mist (`#f2f4f5`) using a `20px` radius border transition.
  - [ ] Integrate standard paths: `/` (Home), `/checkout` (Cart/Checkout redirect), and `/admin` (Dashboard control).
  - [ ] Render a 32px circular profile avatar with a 1px faint border ring at the bottom of the rail.
- **Success Criteria:**
...
- **Task Checklist:**
  - [ ] Create `AppDownloadBanner` fixed/sticky at the top edge of the main page content wrapper.
  - [ ] Configure backgrounds to Ink Black (`#000000`), height to 48px, containing a 24px rounded application thumbnail placeholder.
  - [ ] Render the centered text: "Download Shop app" link styled with Shopify Sans fonts, paired with "Available on iOS & Android" subtext at 10px.
  - [ ] Incorporate the white right chevron indicator.
  - [ ] Wire the close icon button to save a dismissal state to session/local storage to hide the banner persistently.
- **Success Criteria:**
...
- **Task Checklist:**
  - [ ] Configure a `HeroHeader` container using Framer Motion's `useScroll` and `useTransform` hooks.
  - [ ] Map scroll ranges to apply parallax translations and opacity fades to background elements.
  - [ ] Display the "Curated Assets For Premium Creators" display heading in large GT Standard Semibold styling.
  - [ ] Include micro-interactions: element card lists floating and slightly overlapping the logo.
- **Success Criteria:**
...
- **Task Checklist:**
  - [ ] Build a search input container with `9999px` radius, Pure White fill, and hairline borders (0.1 opacity border).
  - [ ] Implement text values mapping to a client-side filter state.
  - [ ] Render a 48px circular submit button in Shop Violet (`#5433eb`) on the right margin.
  - [ ] Apply the specialized shadow token `0 4px 24px rgba(69,36,219,0.34)` to highlight the submit button.
- **Success Criteria:**
...
- **Task Checklist:**
  - [ ] Render a horizontal flex container displaying the four product category tags.
  - [ ] Style each tag as a pill (`9999px` border-radius) with Pure White backgrounds, a hairline border, and a subtle shadow lift (`rgba(0,0,0,0.06) 0 2px 8px`).
  - [ ] Affix circular 16px brand icons containing colors mapped to corresponding classifications.
  - [ ] Set active toggles: selecting a category highlights its boundaries using a Shop Violet hairline tint or active background wash.
- **Success Criteria:**
...
- **Goal Statement:** Formulate the card list displaying the 20 seeded catalog products with bleed layouts and dual-layer shadows.
- **Task Checklist:**
  - [ ] Create a `ProductGrid` wrapping `ProductCard` components.
  - [ ] Design the card with `28px` border radius, Pure White surface, and no visible outer card padding.
  - [ ] Bleed the product image to the top edges using a custom `20px` inner radius configuration.
  - [ ] Apply dual-layer shadow tokens: `0 4px 6px -1px rgba(0,0,0,0.1)` combined with `0 2px 4px -2px rgba(0,0,0,0.1)`.
  - [ ] Render details: Product name in GT Standard Semibold, price, tag badges, and review counts in micro captions.
- **Success Criteria:**
...
#### Step 3.1: Product Detail Layout
- **Duration Estimate:** 2.5 Hours
- **Goal Statement:** Create the dynamic details screen utilizing full-bleed hero visuals and thumbnail strips.
- **Task Checklist:**
...
- **Success Criteria:**
  ...
  - Desktop viewports split layout matrices into sticky info tables and scrolling panels.
- **Cross-References:**
...
- **Task Checklist:**
  ...
  - [ ] Build a mock submit handler: on successful validate, push mock order data to Supabase (or local storage collections), reset cart state, and redirect to the confirmation tracking path.
- **Success Criteria:**
...
#### Step 4.2: Interactive Order Timeline
- **Duration Estimate:** 2 Hours
- **Goal Statement:** Develop the visual order progression timeline displaying tracking statuses.
- **Task Checklist:**
  ...
  - [ ] Structure responsive visual flows: horizontal timeline on desktop, shifting automatically to vertical alignments on mobile viewports.
...
- **Success Criteria:**
  ...
  - Layout conforms cleanly to mobile narrow viewports.
- **Cross-References:**
...
#### Step 4.3: Secure Admin Control Board
- **Duration Estimate:** 3 Hours
- **Goal Statement:** Build the secure dashboard to manually control simulated order progression milestones.
- **Task Checklist:**
  ...
  - [ ] Implement redirect guards returning unauthorized users back to `/login` or displaying access-denied banners.
  - [ ] Render a grid layout listing all mock orders retrieved from store records.
  - [ ] Add pill-shaped status update buttons to trigger status shifts (`Ordered` -> `Shipped` -> `Out for Delivery` -> `Delivered`).
- **Success Criteria:**
  - Navigating to `/admin` without admin credentials blocks page access.
  - Admins can cycle tracking stages, instantly updating active customer order timeline pages.
- **Cross-References:**
